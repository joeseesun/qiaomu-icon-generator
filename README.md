...
这条路径会输出可编辑 SVG：`option-01.svg` 到 `option-08.svg`、`contact-sheet.svg`、`choices.md` 和 `qm-icon-options.json`。如果本机装了 Playwright，可加 `--png` 让 CLI 直接渲染 PNG；否则保留 SVG 也能继续评审和导出。示例见 [`docs/assets/examples/qiaomu-music/svg-cli/`](docs/assets/examples/qiaomu-music/svg-cli/)。
