...
| 方法 | 适合场景 | 输出 |
|---|---|---|
| QM Icon Studio CLI | 需要稳定、干净、矢量友好的 SVG/PNG 候选 | `option-*.svg/png`、`contact-sheet` |
| Codex bitmap reference method | 需要更像 iOS App Store 的精致 raster 图标，或要参考本地 icon gallery | `option-*.png`、`ios-1024/`、`web-32/64/180/` |
...
一次真实使用中，这个 skill 为乔木音乐生成了 10 个候选：

- 候选目录：`design/qiaomu-music-icon-options/`
- 用户选择：`option-05`
- 视觉方向：黑底、琥珀唱片、抽象 Q / 唱臂切口
- Web 输出：`public/icons/qiaomu-music-icon-*.png`
